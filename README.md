...
- **`scripts/new-wiki.sh`** — Spawns a new wiki from `wiki-base/` into a target directory. Creates the expected subdirectories (entities, concepts, sources, comparisons, raw/assets) with `.gitkeep` files. Pass `--git` to initialize (or reuse) a git repo and commit the skeleton, `--force` to overwrite an existing target, or `--into` to merge the skeleton into an existing directory without clobbering any files that are already there. `install.sh` symlinks this as `llm-wiki-new` on your `PATH`; direct invocation is the manual-install path.
