...
Do not commit, do not run lint, do not start a follow-on ingest. The human reviews. Corrections go to the appropriate conventions file — domain-general patterns to `CLAUDE.md` "Wiki Conventions (Domain-General)", domain-specific patterns to `wiki/conventions.md` (you can offer to draft them as a separate step).
